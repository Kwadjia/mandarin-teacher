...
EX['grammar-sense'] = {
  ...
  blurb:'Two versions of the same sentence, one with the words in the wrong order. Chinese '+
        'grammar is mostly word order, so this tests the one thing vocabulary drilling never '+
        'touches. Distractors are generated by permuting real sentences, so it costs nothing '+
        'to produce.',
  tags:['grammar','word order','auto-graded'],
  run(el){
    let it, tries=0;
    do{ it=nextItem(); tries++; }while(it.w.length<4 && tries<40);
    // Swap two adjacent tokens to make a plausible-but-wrong variant.
    const bad=[...it.w];
    const j=Math.floor(Math.random()*(bad.length-1));
    [bad[j],bad[j+1]]=[bad[j+1],bad[j]];
    const right=it.w.join(''), wrong=bad.join('');
    if(right===wrong){ return EX['grammar-sense'].run(el); }
    const opts=shuffle([{t:right,ok:true},{t:wrong,ok:false}]);
    S.t0=performance.now();
    el.innerHTML=`<p class="hint">Which is correct Mandarin?</p>
      <div class="opts" id="o"></div><div id="out"></div>`;
    opts.forEach(o=>{
      const b=document.createElement('button'); b.className='b opt';
      ...
      b.onclick=()=>{
        ...
        const c=clip(it);
        play(c.f);
        $('#out').innerHTML=`<p class="py">${it.p}</p><p class="en">${it.g}</p>
          <p class="hint">Wrong version swapped
            <b>${it.w[j]}</b> and <b>${it.w[j+1]}</b>.</p>
          <div class="big"><button class="b" onclick="play('${c.f}')">▶ Hear it</button>
          <button class="b p" onclick="start('grammar-sense')">Next →</button></div>`;
      };
      $('#o').appendChild(b);
    });
  }
...
function start(id){
  ...
  $('#ex').innerHTML = `<div class="exhdr"><h2>${ex.name}</h2><p>${ex.blurb}</p>
    ...
    </div><div class="stage" id="stage"></div>`;
  ex.run($('#stage'));
}
